...
<div id="recordPaymentModal" class="fixed inset-0 z-50 hidden overflow-hidden" style="position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; bottom: 0 !important; margin: 0 !important; z-index: 50 !important;">
    ...
    <div class="relative z-10 flex min-h-full items-center justify-center p-4 overflow-y-auto overflow-x-hidden">
        <div class="bg-white rounded-xl shadow-none w-full max-w-2xl mx-auto flex flex-col max-h-[90vh]" style="max-width: 42rem;">
            ...
            <div class="flex-1 overflow-y-auto overflow-x-hidden">
                <form method="post" action="<?php echo htmlspecialchars($baseUrl); ?>/purchases/record-payment" id="recordPaymentForm" enctype="multipart/form-data" class="p-6">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(Csrf::token()); ?>">
            ...
            <div class="flex-shrink-0 p-6 pt-4 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row justify-end gap-2 md:gap-3">
                    ...
                    <button type="submit" form="recordPaymentForm" id="submitRecordPaymentBtn" class="w-full sm:w-auto inline-flex items-center justify-center gap-1 md:gap-1.5 bg-blue-600 text-white px-2.5 md:px-3 lg:px-4 py-1.5 md:py-2 rounded-lg hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors font-semibold shadow-sm disabled:opacity-60 disabled:cursor-not-allowed text-xs md:text-sm">
...
/* Record Payment Modal scrollbar styling */
#recordPaymentModal .overflow-y-auto {
  scrollbar-width: thin; /* Firefox */
  scrollbar-color: rgb(203 213 225) transparent; /* Firefox: thumb and track */
}

#recordPaymentModal .overflow-y-auto::-webkit-scrollbar {
  width: 8px; /* Chrome, Safari, Opera */
}

#recordPaymentModal .overflow-y-auto::-webkit-scrollbar-track {
  background: transparent;
  border-radius: 4px;
}

#recordPaymentModal .overflow-y-auto::-webkit-scrollbar-thumb {
  background-color: rgb(203 213 225); /* gray-300 */
  border-radius: 4px;
  border: 2px solid transparent;
  background-clip: content-box;
}

#recordPaymentModal .overflow-y-auto::-webkit-scrollbar-thumb:hover {
...
const INGREDIENTS = <?php echo json_encode(array_map(function($i){ return ['id'=>(int)$i['id'],'name'=>$i['name'],'unit'=>$i['unit'],'display_unit'=>$i['display_unit'] ?? '', 'display_factor'=>(float)($i['display_factor'] ?? 1)]; }, $ingredients), JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES); ?>;
  ...
  const openPurchaseModal = (id) => {
    ...
    overlay.addEventListener('click', function(e) {
      ...
      // Check if clicking the View Transactions button or its children - don't stop propagation
      const viewTransactionsBtn = target.closest('.viewTransactionsBtn');
      if (viewTransactionsBtn) {
        return; // Let the button's event handler work
      }
